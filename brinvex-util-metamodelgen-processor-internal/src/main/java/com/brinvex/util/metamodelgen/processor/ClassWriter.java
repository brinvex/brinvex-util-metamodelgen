...
public final class ClassWriter {
    ...
    private static final ThreadLocal<SimpleDateFormat> SIMPLE_DATE_FORMAT = ThreadLocal.withInitial(() -> new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ"));

    private ClassWriter() {
    }

    public static void writeFile(MetaPojo entity, Context context) {
        try {
     ...
     * @return body content
     */
    private static StringBuffer generateBody(MetaPojo entity, Context context) {
    ...
    private static void printClassDeclaration(MetaPojo entity, PrintWriter pw, Context context) {
    ...
    private static String findMappedSuperClass(MetaPojo entity, Context context) {
        TypeMirror superClass = entity.getTypeElement().getSuperclass();
        //superclass of Object is of NoType which returns some other kind
        while (superClass.getKind() == TypeKind.DECLARED) {
            final Element superClassElement = ((DeclaredType) superClass).asElement();
            String superClassName = ((TypeElement) superClassElement).getQualifiedName().toString();
            if (extendsSuperMetaModel(superClassElement, context)) {
     ...
     */
    private static boolean extendsSuperMetaModel(Element superClassElement, Context context) {
        // if we processed the superclass in the same run we definitely need to extend
        String superClassName = ((TypeElement) superClassElement).getQualifiedName().toString();
        return context.containsMetaEmbeddable(superClassName);
    }

    private static String getFullyQualifiedClassName(MetaPojo entity, String metaModelPackage) {
    ...
    private static String writeGeneratedAnnotation(MetaPojo entity, Context context) {
        ...
        generatedAnnotation.append("@")
                ...
                .append("(value = \"")
                .append(MetamodelGenerator.class.getName());
